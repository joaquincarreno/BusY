...
def fillTimes(group):
    ...
    group["dt"] = diffs.apply(
        lambda x: x.total_seconds() / 60 / 60
    )  # dt en segundos->minutos->horas
    return group
...
def setupSpeeds(skip=True, force=False):
    ...
    if force:
        llaves = (
            GPSRegistry.objects.all().values_list("recorrido", "sentido").distinct()
        )
    else:
        llaves = (
            GPSRegistry.objects.all()
            .filter(speed=None)
    ...
    for r, s in llaves:
        ...
        df.set_geometry(
            [Point(xy) for xy in zip(df["longitude"], df["latitude"])], inplace=True
        )
        df.drop(
            columns=[
                "speed",
        ...
        grouped_df = (
            grouped_df.apply(fillDistances)
            .reset_index(drop=True)
            .drop(columns=["geometry", "timestamp"])
            .groupby("patente")
        )
        grouped_df = (
            grouped_df.apply(fillSpeeds)
            .reset_index(drop=True)
            .drop(columns=["dt", "distancia", "patente"])
        )

        grouped_df.apply(saveSpeeds, axis=1)
        current_step += 1
        print("done, progress:", 100 * current_step / total_steps)
    print("total time elapsed: {time}".format(time=round(time() - t0, 1)))
