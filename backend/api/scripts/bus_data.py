...
def filter_points(points_df):
    ...
    print(
        "se liberaron {} puntos para la patente {}".format(
            len(points_df) - len(filtered), points_df["patente"].iloc[0]
        )
    )
    if len(filtered) > 2:
        return filtered
...
def process_file(file_name):
    ...
    ddf = (
        ddf[
            ddf.columns.intersection(["patente", "recorrido", "datetime", "lat", "lon"])
        ]
        .replace(" ", np.nan)
        .dropna()
    )
...
def read_gps(filename, sample=False):
    ...
    print(
        str(len(gps) - len(filtered))
        + " buses with less than {filter} gps entry".format(filter=min_count)
    )
...
def setupGPSEntries(refill=False):
    ...
    if GPSRegistry.objects.count() > 0:
        return
    ...
    for f in files:
        ...
        for i in data.index:
            ...
            if progress > limiter:
                ...
                print(
                    "progress: " + str(100 * progress) + "%, elapsed time",
                    round(time() - t0, 1),
