...
def load_data():
    ...
    # arreglar mayúsculas
    df["Sentido Servicio"] = df["Sentido Servicio"].apply(
        lambda x: x if x != "ida" else "Ida"
    )
...
def setupRoutes(refill=False):
    ...
    def fill_model(row):
        try:
            ...
            print(
                "Multiples stops found with the TS code {ts_code}".format(
                    ts_code=row["Código paradero TS"]
                )
            )
