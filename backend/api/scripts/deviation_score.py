...
def pointListToGdf(points, data=None):
    ...
    gdf = gpd.GeoDataFrame(data=data, geometry=[Point(x[0], x[1]) for x in points], crs=CRS_ORIG)
...
def setupDeviations(skip=True, refill=False, clear_scores=False):
    ...
    for r, s in llaves:
        ...
        print(r, s)
        try:    
            paradas = list(Routes.objects.filter(serviceTSCode=r, serviceDirection=s).order_by('order').values_list('stop', flat=True))
